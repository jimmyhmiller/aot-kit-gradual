...
| NavierStokes | addPoints FIRES as of 9b6b26c (the blocker was callback pinning, NOT the equality); density VALUES still wrong | The real blocker was fng-local-function-target pinning `uiCallback()` to its initial empty closure (15-line repro pf2.js). Fixed with the mutable-FeCapture guard. The old equality-lowering theory is DEAD: MI-JSUNBOX flt converts int-tagged words at runtime (geq.js proves it), and routing that branch through guarded ToNumber regresses DeltaBlue selection (Fun-as-value MSEL at a structural compare site) — do not retry it blindly. CURRENT state on ns-small.js (16x16, 2 iters, 1 frame, scratchpad): native checksum 100 vs Node 68; pfCalls and framesBetweenAddingPoints match Node exactly (probe: ns-probe2.js). The evaluator runs 65k steps into ns-small then fails EV-TYPE at an ArrayLoad (n4059, inside the sim's array code) — fixing that eval gap gives a semantic single-step view of the density divergence. Full-size ns.js: Node checksum 26.|
