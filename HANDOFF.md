...
| NavierStokes | RUNS THE WHOLE BENCHMARK END TO END (setup, one update, capture, teardown; exit 0, no crash); checksum wrong (returns tagged int 0, Node expects 2762) | Crash-free as of the captured-parameter cell fix (the first attempt stored the boxed parameter INTO ITSELF because the raw Parm in values[] was mistaken for an existing cell; clearing the binding first routes fng-set-symbol! through cell creation). The work remaining is pure CORRECTNESS: nsChecksum comes out 0 — either the density writes (addPoints -> setDensity through the Field object) or the reads (getDensity in nsCapture) lose the values, or the arithmetic drops them; and the kernel result arrives as a TAGGED int (0x7FFC<<48) where the harness expects raw. Repro: scratch harness `navier-stokes.coil` -> emitter (~3 min) -> clang link with tools/native-gc-runtime.c + trampoline + v8-native-harness -> `ns-bin 268435456` prints `result=9222246136947933184` (= tagged 0). Expected 2762. The `js_array_reserve` absurd-growth tripwire stays. |
