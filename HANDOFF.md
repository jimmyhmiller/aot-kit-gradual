...
| NavierStokes | Compiles, emits, links, RUNS deep into the simulation; ONE register-allocator bug remains, precisely diagnosed | The program executes through construction and reset (arrays correctly 130x130=16900) into the update loop's inner solver. The remaining bug is in REGISTER ALLOCATION: in the `x[cr]=x0[cr]; y[cr]=y0[cr]` fast path (lin_solve2-shaped, `aot_10+2160..2260` in the current build), the index `currentRow` is kept in **x8 across two runtime calls** (`aot_arr_load` then `aot_arr_store`). x8 is caller-clobbered (bit 8 IS in the effect-class-3 clobber mask `0xffff00ff00000fff`, and MI-JSARRAY IS effect class 3), so either `mra-build-interference!`'s live-across-call forbidding (backend_allocate.coil:193) has a hole, or the OTHER allocation path (`be-use-runtime-allocation! true`, seeded scheduling) skips the clobber constraint. After the call, x8 holds a leftover heap pointer and the store index becomes a pointer — caught instantly by the `js_array_reserve` absurd-growth tripwire (>2^28 elements → print + `__builtin_trap`) now in tools/native-gc-runtime.c; keep it. Repro: scratch harness `navier-stokes.coil` → emitter (~3 min) → `clang tools/native-gc-runtime.c tools/native-gc-trampoline.S tools/v8-native-harness.c ns.o -o ns-bin` → `ns-bin 268435456` (first arg is HEAP BYTES; `ns-bin 0 10` = zero-byte heap = exit 86). lldb the trap, `frame select 2`, disassemble — the `add x15, x8, xzr` index moves around the two `bl` sites show it directly. Expected checksum 2762. |
