...
| NavierStokes | Runs through setup, reset, dispatch, into the update/display path; one capture-slot representation bug remains | Fixed since the last row update: (1) FP-computed array indexes selected into FPR vregs and the JSARRAY emitter encoded d8's number as x8 — `ms-array-index!` now converts FP indexes via MI-I32FROMFP at every array-op selection site; (2) the poly dispatcher ignores the finite target summary (closed-world inference cannot see argument flow through dynamic method calls into capture cells, so summary chains missed the runtime callee) and dispatches over every unit function; (3) `mu-build-program!` builds EVERY live Fun into the unit — a callable whose identity escapes only as a value (nsCapture through `setDisplayFunction`) had no call edge and was absent from the dispatch chain. FIXED since: captured PARAMETERS now live in cells (`Field(dens,u,v)`'s params were bound as raw Parms, so nested closures stored the TAGGED parameter value into cell-typed env slots and readers dereferenced it as a cell — the `0xfff8...`-tagged slot was `Box(array)`). REMAINING: the same class one layer on — with array values now flowing TAGGED out of cells, an INLINE array fast path dereferences the ARRAY-tagged word without an array-unbox (crash at `aot_b+14364` in the current scratch build, `EXC_BAD_ACCESS 0xfff8000464000a78` = boxed-array payload + offset, path setup → setResolution → reset-ish fill loops). The fix shape: wherever fng's element/array fast paths consume an object the frontend PROVED to be an array but whose value arrives as dyn (a cell load), emit `OP-ARRAYUNBOX` (JSUNBOX with JSV-ARRAY) before the shaped access — mirroring the `lexical-this` unbox fix in `fng-field-load-value` and using structural (which-node) evidence, not build-time type identity. Repro: scratch harness `navier-stokes.coil` → emitter (~3 min) → `clang tools/native-gc-runtime.c tools/native-gc-trampoline.S tools/v8-native-harness.c ns.o -o ns-bin` → `ns-bin 268435456` (first arg is HEAP BYTES; `ns-bin 0 10` = zero-byte heap = exit 86 via the OOM trap). The `js_array_reserve` absurd-growth tripwire (>2^28 elements → print + `__builtin_trap`) in tools/native-gc-runtime.c stays. Expected checksum 2762. |
